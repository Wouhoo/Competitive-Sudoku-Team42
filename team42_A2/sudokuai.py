...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        current_depth = 1
        while True:
            self.propose_move(_find_best_move(game_state, current_depth, pruned))
            current_depth += 1


def _find_best_move(game_state: GameState, depth: int, pruned) -> Move:
    ...
    for i, move in enumerate(good_moves):
        ...
        score = _minimax_alphabeta(
            new_state, depth=depth - 1, is_maximizing=not is_maximizing, pruned=pruned
        )
        scores[i] = score
    ...
    if is_maximizing:
        ...
        for move in good_moves:
            ...
            value = max(
                value,
                _minimax_alphabeta(
                    new_state, depth - 1, alpha, beta, not is_maximizing, pruned
                ),
            )
